...
2. **Socket location inconsistency**: aligned by setting daemon default to `/tusk/vm/serial.sock` (host target `~/.tusk/vm/serial.sock`).
...
- Simulate malformed JSON-RPC request to `/tusk/vm/serial.sock` to ensure no panic in `tuskd`.
